    ...
    if let Some(input_filepath) = arguments.filepath {
        ...
        if !rad_filepath.is_file() {
            if input_filepath.is_file() {
                return error(&format!("File found but no '.rad' file found: {:?}", rad_filepath), 1)
            } 
            ...
            Some(p) => match p.is_dir() {
                ...
                false => {
                    if let Some(parent) = p.parent() {
                        if !parent.is_dir() {
                            return error(&format!("Output directory of path is not a directory: {:?}", p), 1);
                        };
                    };
                    p.clone()
                }
        ...
        let mut gpr = match gpr::GPR::from_meta_and_loc(gpr_locations, gpr_meta) {
            Ok(g) => g,
            Err(e) => return error(&format!("Error loading GPR data from {:?}: {:?}", rad_filepath.with_extension("rd3"), e), 1)
        };
        ...
        let profile: Vec<String> = match arguments.default {
            ...
            false => match &arguments.steps {
                ...
                None => return error(&format!("No steps specified. Choose a profile or what steps to run"), 1)
            },
        };
